...
const logger = require('pino')({ level: LOG_LEVEL, name: 'SaigenRecognizeStream' });

const WEBSOCKET_CONNECTION_ERROR = 'WebSocket connection error';
const ERROR_UNRECOGNIZED_FORMAT = 'UNRECOGNIZED_FORMAT';
...
/*     if (options.objectMode) {
    ...
    this.on('newListener', (event) => {
      logger.debug(`received newListener: ${event}`);
      if (!options.silent) {
        if (
          event === 'results' ||
            event === 'result' ||
  ...
  initialize() {
    ...
    const url =
        ...
        }?content-type=audio%2Fx-mulaw%2C+layout%3D%28string%29interleaved%2C+rate%3D%28int%298000%2C+format%3D%28string%29S16LE%2C+channels%3D%28int%291`;

    console.log(`Using WS URL: ${url}`);
    ...
    self.once('open', self.listen.bind(self));
    ...
    socket.onerror = (event) => {
      ...
      err.name = WEBSOCKET_CONNECTION_ERROR;
      err.event = event;
      self.emit('error', err);
      self.push(null);
    };
    ...
    this.socket.onopen = () => {
      ...
      /**
         ...
         */
      logger.debug(`onopen: ${openingMessage}`);
    ...
    socket.onmessage = (frame) => {
      ...
      } else if (options.readableObjectMode) {
        /**
               ...
               */
        self.push(data);
        logger.debug('Pushed data to self');
      ...
      if (data.result) {
        // send results back to STT Adapter
        // from saigen: { hypotheses: [ { transcript: '<unk>' } ], final: false }
        console.log(data.result);
        // transform
        const sttOutput = { results: [{ alternatives: [{ transcript: data.result.hypotheses[0].transcript }], final: data.result.final }] };
        self.emit('data', sttOutput);
      }
    };
  ...
  _write(chunk, encoding, callback) {
    //logger.debug(`_write event ${encoding} - ${this.options['content-type']}`);
    const self = this;
    if (self.finished) {
    ...
    if (!this.initialized) {
      if (!this.options['content-type']) {
        const ct = this.getContentType(chunk);
        if (ct) {
          this.options['content-type'] = ct;
        } else {
          logger.debug(`Unable to determine content-type from file header, please specify manually.`);
          const err = new Error('Unable to determine content-type from file header, please specify manually.');
          err.name = ERROR_UNRECOGNIZED_FORMAT;
          this.emit('error', err);
          this.push(null);
          return;
        }
      }
      logger.debug('Intilize started!');
      this.initialize();
      logger.debug('Intilize completed!');
      this.once('open', () => {
        self.sendData(chunk);
  ...
  listen() {
    const self = this;
    self.emit('message', { state: 'listening' });
    logger.debug('emitted listening');
